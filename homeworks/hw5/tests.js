...
CONFIG.isStep(steps.footer) && describe("Footer",() => {
    ...
    მეორეში, ჩაწერე "Ⓒ თბილისის თავისუფალი უნივერსიტეტი, ${new Date().getFullYear()}". (Ⓒ სიმბოლოს ჩასასმელად შეგიძლია ჩაწერო &#9400, 
    ანუ სადაც &#9400 ჩაწერ მაგ ადგილს HTML აღიქვამს როგორც Ⓒ სიმბოლოს). არ დაგავიწყდეს გაცენტრვა და მძიმესა და ${new Date().getFullYear()}-ს შორის გამოტოვების ჩასმა.
    ხოლო მეორე პარაგრაფში რამე მესიჯი დამიწერე :)`,() => {
        const footer = document.querySelector("div#footer");
...
CONFIG.isStep(steps.done) && describe("🥳 Success", () => {
    it("მშვენიარია, პირველი დავალება ბოლომდე მიიყვანე!", () => {})
    toggle('help')
